/*
  File: /js/app.js
  Title: Wasaya — App Logic (multi-page, slider enabled in home)
*/
(() => {
  ...
  // بيانات
  const state = { dom:[], dtm:[], wills:[], alerts:{wills:false,debts:false}, plan:null, theme:'dark' };
  ...
  const load = () => {
    ...
    state.theme = localStorage.getItem(key('theme')) || 'dark';
  };
  ...
  function ensureLogin(){
    ...
    wrap.innerHTML = `
      <div class="auth-card">
        <h3>تسجيل الدخول</h3>
        <p class="tiny muted">اكتب رقم جوالك لبدء استخدام التطبيق.</p>
        <label>رقم الجوال</label>
        <input id="login-phone" type="tel" inputmode="numeric" placeholder="05xxxxxxxx" />
        <button id="login-go" class="btn primary full" style="margin-top:10px">دخول</button>
      </div>`;
    ...
    $('#login-go').onclick = ()=>{
      ...
      load(); document.body.removeChild(wrap); initPage();
    };
  }
  ...
  function applyTheme(){
    ...
    $('#themeToggle') && ($('#themeToggle').textContent = (state.theme==='light' ? '🌞' : '🌙'));
  }
  ...
  // قائمة إضافة الدين (الرئيسية فقط)
  ...
  function renderDebts(list, mountId, typeLabel){
    const mount = document.querySelector(`#${mountId}`); if(!mount) return;
    mount.innerHTML = '';
  ...
  function openDebtDialog(it, list, typeLabel){
    ...
    $('#dlg-del-debt').onclick = ()=>{
      if(confirm('حذف هذا الدين؟')){
        const idx = list.findIndex(x=>x.id===it.id);
        list.splice(idx,1); save(list===state.dom?'dom':'dtm'); rerenderDebts(); dlg.close();
      }
    };
    $('#dlg-add').onclick = ()=>{
  ...
  function editDebt(it, list){
    ...
    document.querySelector(`#${pref}-name`)?.scrollIntoView({behavior:'smooth', block:'start'});
    document.querySelector(`#${pref}-name`).value = it.name || '';
    document.querySelector(`#${pref}-phone`).value = it.phone || '';
    document.querySelector(`#${pref}-amount`).value = it.amount || '';
    const idx = list.findIndex(x=>x.id===it.id);
    if(idx>-1) list.splice(idx,1);
    save(list===state.dom?'dom':'dtm'); rerenderDebts();
  }
  function rerenderDebts(){
    renderDebts(state.dom, 'dom-list', 'دين علي');
    renderDebts(state.dtm, 'dtm-list', 'دين لي');
  }

  // حفظ ديون
  document.querySelector('#form-debt-on-me')?.addEventListener('submit', async e=>{
    e.preventDefault();
    const name=document.querySelector('#dom-name').value.trim(),
          phone=document.querySelector('#dom-phone').value.trim(),
          amount=document.querySelector('#dom-amount').value.trim();
    const docs=await filesToEntries(document.querySelector('#dom-files'));
    state.dom.push({id:uid(), name, phone, amount, docs, payments:[], created:today()});
    save('dom'); e.target.reset(); rerenderDebts();
  });
  document.querySelector('#form-debt-to-me')?.addEventListener('submit', async e=>{
    e.preventDefault();
    const name=document.querySelector('#dtm-name').value.trim(),
          phone=document.querySelector('#dtm-phone').value.trim(),
          amount=document.querySelector('#dtm-amount').value.trim();
    const docs=await filesToEntries(document.querySelector('#dtm-files'));
  ...
  $('#privacy')?.addEventListener('click', e=>{
    e.preventDefault();
    openContentDialog('سياسة الخصوصية', (site().privacy||'لم تتم إضافة سياسة بعد.').replace(/\n/g,'<br>'));
  });
  $('#contact')?.addEventListener('click', e=>{
    e.preventDefault();
    openContentDialog('تواصل معنا', (site().contact||'لم تتم إضافة معلومات الاتصال بعد.').replace(/\n/g,'<br>'));
  });

  /* ✅ جديد: عرض الأسئلة الشائعة بشكل أكورديون جميل */
    ...
    const html =
      ...
      data.map(x=>`
        <details class="faq-item">
          <summary><span class="chev">▾</span> ${esc(x.q)}</summary>
          <div class="faq-answer">${esc(x.a).replace(/\n/g,'<br>')}</div>
        </details>
      `).join('') +
      '</div>';
...
/* === قراءة إعدادات المسؤول العامة وتطبيقها في تبويب "المزيد" === */
(function(){
